...
function deleteItem($db){
    ...
    if(HASHTAG_ALLOWED_ADMIN && !empty($thisId)){
        if (mysqli_query($db_con, "UPDATE ".$db['media_table']." SET approved = 0 WHERE id = ".$thisId)){
            $result["success"] = true;
        }else{
...
function updateInstagram($db, $instagram, $hashtag){
    ...
    // get the last id
    $query = mysqli_query($db_con, "SELECT * FROM ".$db['media_table']." WHERE hashtag='$hashtag' AND source='instagram' ORDER BY source_id DESC LIMIT 1");
    $result = mysqli_fetch_array($query);
